...
\end{equation}
One can also replace all occurrences of $E(x)$ by the classical error term $O(x \exp(-c \sqrt{\log x}))$ for some absolute constant $c>0$ (in which case the $\frac{2}{\sqrt{t}}$ type terms can be absorbed into the error term).
\end{lemma}
...
This procedure works well for small $N$, for instance attaining the exact value of $t(N)$ for all $N \leq 79$, though it begins to degrade for larger $N$; see \Cref{fig-zoom}.  Its performance is also respectable (though not optimal) for medium $N$; for instance, when $N=3 \times 10^5$ and $t=N/3$, it establishes the lower bound $M(N,t) \geq N+372$, which is close to the exact value of $M(N,t) = N+455$ we are able establish using the linear programming methods of the next section (see \eqref{mnt}).

Our implementation of the greedy algorithm relies on three observations. The first is that for large primes $p|N$, we expect to use factors of the form $mp$ with $m=\lceil t/p\rceil$ minimal. This is obvious for $p > t$ (where $m=1$), and more generally, if we fix a small prime $\ell < M$ and consider
\begin{equation}\label{eq:gval}
v_\ell\left(\prod_{M<p\le N} \left\lceil\tfrac{t}{p}\right\rceil^{v_p(N!)}\right),
\end{equation}
we expect this to be less than $v_{\ell}(N!)\approx \frac{N}{\ell-1}$ for $M\ge N^{\nicefrac{1}{2}}$, and in practice we find this is always true for $M\ge \sqrt{t}$ (our implementation verifies this assumption).

Assuming this, we partition the range $(M,N]$ into subintervals $I_{m,n}=(a_{m,n},b_{m,n}]$ such that for every prime $p\in I_{m,n}$ we have $\lceil \nicefrac{t}{p}\rceil=m$ and $v_p(N!)=n$.  We can then compute the number of factors produced by the greedy algorithm that are divisible by a prime $p>M$ as
\begin{equation}\label{eq:picount}
\sum_{m,n} n\left(\pi(b_{m,n})-\pi(a_{m,n})\right).
\end{equation}
If $\lceil \nicefrac{t}{p}\rceil=m$ and $v_p(N!)$ with $t\in (\nicefrac{N}{4},\nicefrac{N}{2})$ and $p> N^{\nicefrac{1}{2}}$, then $n = \lfloor \nicefrac{N}{p}\rfloor\in [2m-1,4m]$, and it follows that the sum contains $O(N^{\nicefrac{1}{2}})$ terms.  There are analytic methods to compute $\pi(x)$ in $O(x^{\nicefrac{1}{2}+\epsilon})$ time \cite{lagarias-odlyzko}, which in combination with sieving primes up to $N^{\nicefrac{3}{4}}$ would allow one to compute \eqref{eq:picount} in $O(N^{\nicefrac{3}{4}+\epsilon})$ time. However, most practical implementations for computing $\pi(x)$ use the $O(x^{\nicefrac{2}{3}+\epsilon})$ algorithm described in \cite{deleglise-rivat,lagarias-miller-odlyzko}.  In this case the optimal asymptotic approach is to sieve primes up to $N^{5/6}$, yielding an $O(N^{\nicefrac{5}{6}+\epsilon})$-time algorithm. In our implementation we used a  slightly smaller sieving bound to better balance the time spent sieving primes versus counting them in the range $N\le 10^{12}$ using the \texttt{primesieve} \cite{walisch-primesieve} and \texttt{primecount} \cite{walisch-primecount} libraries that we employed in our implementation.

If we precompute the prime factorizations of the $m\le \nicefrac{t}{M}$, we can compute \eqref{eq:gval} each prime $\ell \le M$ and subtract this value from $v_\ell(N!)$ to obtain the prime factorization of the divisor of $N!$ that remains after removing all factors divisible by a  prime $p>M$ (at which point we check that the remaining valuations are nonnegative to confirm our assumption above).  It then remains only to partition this remaining $M$-smooth factor of $N!$ into factors of size at least $t$.

Our second observation is that at any point where the greedy algorithm is attempting to determine  a suitable cofactor $m_p$ for the largest remaining $p$, the cofactor $m_p$ must be $p$-smooth and satisfy $m_pp\ge t$ (and $m$ be constructible from the remaining part of the factorization of $N!$).  This implies that for any prime $q\le p$, every suitable cofactor $m_q$ is also a suitable cofactor for $p$, and since the greedy algorithm always minimizes the choice of $m_p$, we must have $m_p\le m_q$.  This implies that as we consider primes $p$ in decreasing order we can consider cofactors $m_p$ in increasing order.  If we precompute the prime factorizations of all the $m_p$ that may arise (e.g., by sieving the interval $[1,N]$ for $M$-smooth integers), we obtain a quasilinear running time of $O(N^{1+\epsilon})$; note that at each step the algorithm will use the largest exponent $e$ such that $(m_p p)^e$ divides the remaining part of the factorization of $N!$, at which point one of $p$ or $m_p$ is eliminated, and the total number of $p$ and $m_p$ to be considered is $O(N)$. The running time is dominated by the time to precompute the prime factorizations of all the $m_p$, but in a situation where we may be considering many values of $N$ and $t$, this precomputation can be reused.

Our third observation is that there is an additional constraint on $m_p$ beyond being $p$-smooth, which is that the ratio of the largest and smallest prime factors of $m_p$ is bounded by $t/m_p$. If $\ell_1$ and $\ell_2$ are prime divisors of $m_p$ with $\ell_1\le \ell_2\le p$, then $m_p \ell_2 \ge t\ell_1$ implies $(m_p/\ell_1)p\ge t$, in which case $m_p/\ell_1$ is a strictly smaller suitable cofactor for $p$ we could use in place of $m_p$. This observation decreases the number of $m_p$ one needs to consider, which can be used to reduce the memory footprint by a significant constant factor.  This constraint is sensitive to the value of $t$, which constrains the ability to reuse our precomputation of $m_p$, but for large values of $N$, memory is the limiting factor.

\subsection{A fast variant of the greedy algorithm}
We now describe a variant of the greedy algorithm that produces slightly weaker bounds on $t(N)$ in general, but obtains an $O(N^{\nicefrac{5}{6}+\epsilon})$ running time using $O(N^{\nicefrac{5}{8}})$ space (in fact the exponent $\nicefrac{5}{6}$ can be replaced by $\nicefrac{3}{4}$ and the exponent $\nicefrac{5}{8}$ can be replaced by any $c>\nicefrac{1}{2}$).  The algorithm fixes $M$ satisfying $M(M-1)\ge t$ and treats the primes $p\ge M$ as in the first phase of the greedy algorithm described above.  Rather than precomputing a list of suitable $m_p\le N$, the algorithm instead precomputes a list of $M$-smooth integers $m$ up to $N^{\nicefrac{5}{8}}$.  As it considers primes $p < M$ in decreasing order, it will eventually reach appoint where no precomputed value of $m$ is a suitable cofactor for $p$.  When this occurs it will instead look for a cofactor that is suitable for $p^2$, which will be smaller and easier to construct from the remaining part of the factorization of $N!$.  It will continue in this faction to consider suitable cofactors for both $p$ and $p^2$ until it eventually reaches a point where neither can be found, and which point $p$ will be quite small (not much bigger than $N^{\nicefrac{3}{16}}$) at which point the algorithm will simply construct factors of $N!$ larger than $t$ by combining available remaining primes in decreasing order.  This will occasionally result in factors that are larger than the original greedy algorithm would use, but the difference is small and the algorithm can construct these factors more quickly using less memory, which allows it to handle large values of $N$ much more efficiently.
...
\begin{equation}\label{mnt}
  M(N,t) \leq N + 445
\end{equation}
for this choice of $N,t$.  In fact, as discussed later in this section, we know that equality holds in this particular case.  For $N \leq 10^4$, we found that the linear programming upper bound on $t(N)$ is tight except when
...
\begin{itemize}
  ...
  \item For each $k \geq 1$, and each element $m$ of $\tuple$ in the interval $[\alpha N/p_0^{r+k}, \alpha N/p_0^{r+k-1})$, add $p_0^{r+k} m$ to $\tuple'$.
...
We can also recover a weak version of \Cref{main}(iv) with this method:
...
where the $*$ indicates that the final term $m=M$ in the sum is doubled (so that we have $\sum_{0 \leq m \leq M}^* f(m) = \sum_{0 \leq m<M} f(m) + 2 f(M)$).  Meanwhile, the left-hand side can be computed as
\begin{align*}
