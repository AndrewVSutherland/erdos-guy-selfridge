...
The following greedy algorithm is a simple method that gives reasonably good lower bounds on $M(N,t)$:
...
Observation (a) allows us to efficiently handle large $p_i$, observation (b) enables an $O(N^{1+\epsilon})$ running time, and observation (c) allows us to more efficiently handle small $p_i$, which turns out to be the main bottleneck of the simple greedy algorithm sketched above.  In the next subsection we describe a modification of the greedy algorithm that produces slightly weaker lower bounds on $M(N,t)$ with a power savings in the running time; one can achieve $O(N^{\nicefrac{2}{3}+\epsilon})$ asymptotically.  The complexity of our implementation is $O(N^{\nicefrac{3}{4}+\epsilon})$, but it is faster in the range of $N$ relevant to our application.
...
Let $x_0=\lfloor N/M\rfloor < N^{\nicefrac{1}{2}}$, and partition the interval $(M,N]$ into subintervals $(x_i,x_{i+1}]$ on which the step functions $f_t(x)\coloneqq\lceil t/x\rceil$ and $f_N(x)\coloneqq\lfloor N/x\rfloor$ are both constant, such that $x_r=N$, and for $1\le i < r$ the integer $x_i$ is a point of discontinuity for at least one of $f_t(x)$ and $f_N(x)$.  For $0\le i\le r$, let $m_i$ and $n_i$ denote the value of $f_t(x)$ and $f_N(x)$ on the interval $(x_i,x_{i+1}]$, respectively.  Under our assumption that the greed algorithm uses optimal cofacters $m=\lceil t/p\rceil$ for each prime $p>M$, we will have $n_i$ factors of the form $m_ip\ge t$ for each prime $p\in (x_i,x_{i+1}]$, and we can compute the number of factors produced by the greedy algorithm that are divisible by a prime $p>M$ as
\begin{equation}\label{eq:picount}
\sum_{i=1}^r n_i\left(\pi(x_{i+1})-\pi(x_i)\right).
\end{equation}
If $\mathcal B$ is the multiset of the factors $m_ip_i$ for $1\le i\le r$, for primes $p < M$ we can compute
\begin{equation}\label{eq:rval}
v_p(N!/\prod \mathcal B_M) = v_p(N!) - \sum_{i=1}^r n(i)v_p(m_i)
\end{equation}
...
\[
\sum_{i=1}^rn_iv_p(m_i) \approx \frac{1}{p-1}\int_M^t \frac{N}{x\log x}dx \approx\frac{\log 2}{p-1} < \frac{1}{p-1} \approx v_p(N!),
\]
which motivates our observation (a) above. Our algorithm operates on the assumption that $v_p(N!) \ge \sum_{i=1}^rn(i)v_p(m_i)$ for $p>M$, but verifies this as it runs (this verification did not fail in any of our computations, but if it did one could simply increase $M$).

Computing the sum in \eqref{eq:picount} involves computing $\pi(x_i)$ for $O(N^{\nicefrac{1}{2}})$ values of $x_i\in (N^{1/2},N]$.
Up to a constant factor, this is the same as the cost of compute $\pi(N/k)$ for $1\le k \le N^{\nicefrac{1}{2}}$.
...
If we precompute the prime factorizations of the $m\le \nicefrac{t}{M}$, we can compute \eqref{eq:rval} each prime $p \le M$ in $O(N^{\nicefrac{1}{2}+\epsilon})$ time (and verify our assumption that it is nonnegative).  This together with \eqref{eq:picount} is the all the information we need to complete the next phase of the algorithm, which partitions the remaining $M$-smooth factor of $N!$ into factors of size at least $t$.

Here we recall observations (b) and (c) above, that the $m_i$ are nonincreasing and $p_i$-smooth.  This implies that for very $j>i$ we have $m_j\ge m_i$, and that $m_j$ is also $p_i$-smooth, hence a suitable choice for $m_i$.  This means that we can  precompute a table of $M$-smooth integers $m\le t$ and process them in increasing order as we consider decreasing primes $p_i$, thereby obtaining a quasilinear running time of $O(N^{1+\epsilon})$; note that at each step the algorithm will use the largest exponent $e$ such that $(m_p p)^e$ divides the remaining part of the factorization of $N!$, at which point one of $p$ or $m_p$ is eliminated, and the total number of $p$ and $m_p$ to be considered is $O(N)$. The running time is then dominated by the time to precompute the prime factorizations of all the candidate $m_i$.  The additional constraint on the prime factors of the $m_i$ noted in our third observation reduces the number we need to store by a significant constant factor, but asymptotically we still need close to $O(N)$ space, and the $O(N)$ running time is not changed.

\subsection{A fast variant of the greedy algorithm}
We now describe a variant of the greedy algorithm that produces slightly weaker bounds on $t(N)$ in general, but obtains an $O(N^{\nicefrac{3}{4}+\epsilon})$ running time using $O(N^{\nicefrac{2}{3}})$ space (in fact the space can be reduced to $O(N^{\nicefrac{1}{2}+\epsilon})$).  The algorithm fixes $M$ satisfying $M(M-1)\ge t$ and treats the primes $p\ge M$ as in the first phase of the greedy algorithm described above.  Rather than precomputing a list of candidate $m_i\le N$, the algorithm instead precomputes a list of $M$-smooth integers $m$ up to $N^{\nicefrac{2}{3}}$.  As it considers primes $p < M$ in decreasing order, it will eventually reach appoint where no precomputed value of $m$ is a suitable cofactor for $p_i$ (this will certainly happen for $p_i < N^{1/3}$).  When this occurs it will instead look for a cofactor that is suitable for $p^2$, which will be smaller and easier to construct from the remaining part of the factorization of $N!$.  It will continue in this faction to consider suitable cofactors for both $p$ and $p^2$ until it eventually reaches a point where neither can be found, at which point $p=O(N^\epsilon)$ will be quite small.  In the final phase we simply construct factors of $N!$ larger than $t$ by combining available remaining primes in decreasing order.  This will occasionally result in factors that are larger than the original greedy algorithm would use, but the difference is small and the algorithm can construct these factors more quickly using less memory.  This allows it to handle large values of $N$ more efficiently, as can be seen in in the timings in Table~\ref{greedy-table}.
