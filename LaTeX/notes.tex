...
\begin{itemize}
  ...
  \item In \Cref{rearrange-sec}, we extend the \emph{rearrangement approach} from \cite{guy-selfridge} to give a computer-assisted proof that \Cref{main}(iii) holds for sufficiently large $N$, as well as an analytic proof of \eqref{t1}.
...
The following greedy algorithm is a simple method that produces reasonably good lower bounds on $M(N,t)$:
...
The greedy approach works well for small $N$, attaining the exact value of $t(N)$ for all $N \leq 79$, but the quality of the bounds on $t(N)$ it produces begins to decline as $N$ grows; see \Cref{fig-zoom}.  Its performance is also respectable (though not optimal) for medium $N$; for instance, when $N=3 \times 10^5$ and $t=N/3$, it establishes the lower bound $M(N,t) \geq N+372$, which is close to the exact value of $M(N,t) = N+455$ we are able establish using the linear programming methods of the next section (see \eqref{mnt}).

To handle the larger values of $N$ needed to establish Theorem~\ref{main}(iii) for $N\in [ 8\times 10^4, 10^{11}]$ (and also for the broader range $N\in [67245,10^{12}]$ that comfortably overlaps regions where we apply other methods), we now consider how to efficiently implement steps (1) and (2) of the greedy algorithm outlined above.  Let $p_1\ge p_2\ge \ldots \ge 2$ be the prime factors of $N$ listed with multiplicity in non-increasing order, and let $m_ip_i\ge t$ be the factor of $N!$ chosen by the greedy algorithm for the prime $p_i$.  Our implementation of the greedy algorithm is based on the following observations:
...
\item[(b)] The sequence $m_i$ is nonincreasing.
...
In section \ref{sec:fast-greedy} we describe a variant of the greedy algorithm that produces slightly weaker lower bounds on $M(N,t)$, but obtains a power savings in the running time: we show that one can achieve an $O(N^{\nicefrac{2}{3}+\epsilon})$ running time asymptotically. The complexity of our implementation of this faster variant is actually $O(N^{\nicefrac{3}{4}+\epsilon})$, but it is faster than the asymptotically superior approach in the range of $N$ we are concerned with.  While one might suppose that an algorithm whose output is a factorization of $N!$ into $N$ factors would require $\Omega(N)$ time (and $\Omega(N\log N)$ space for the output), we can compress this factorization using tuples of the form $(n,m,p_{\min},p_{\max})$ to represent $n$ occurrences of factors $mp$ for each prime $p\in [p_{\min},p_{\max}]$; for example, the tuple $(\pi(N)-\pi(t-1),1,t,N)$ encodes all factors $p_i\ge t$ in a single tuple.
...
Let $x_0=\lfloor N/M\rfloor < N^{\nicefrac{1}{2}}$, and partition the interval $(M,N]$ into subintervals $(x_k,x_{k+1})$ on which the step functions $f_t(x)\coloneqq\lceil t/x\rceil$ and $f_N(x)\coloneqq\lfloor N/x\rfloor$ are both constant, such that $x_r=N$, and $x_k$ is a point of discontinuity for at least one of $f_t(x)$ and $f_N(x)$ for $0<k<r$.  Under our assumption that the greedy algorithm uses optimal cofacters $m_i=\lceil t/p_i\rceil$ for each $p_i>M$, we will have $f_N(x_k)$ factors $m_ip_i\ge t$ with $m_i=f_t(x_{k+1})$ for each prime $p_i\in (x_k,x_{k+1}]$, and we can compute the number of factors produced by the greedy algorithm that are divisible by a prime $p>M$ as
\begin{equation}\label{eq:picount}
\sum_{k=0}^{r-1} f_N(x_k)\left(\pi(x_{k+1})-\pi(x_k)\right).
\end{equation}
If $\mathcal B$ is the multiset of the factors $m_ip_i$ with $p_i\ge M$, for primes $p < M$ we can compute
\begin{equation}\label{eq:rval}
v_p(N!/\prod \mathcal B) = v_p(N!) - \sum_{k=0}^{r-1} f_N(x_k)v_p(f_t(x_{k+1}))
\end{equation}
using a precomputed table of factorizations of integers $m < N^{\nicefrac{1}{2}}$ and $v_p(N!)=\sum_{e=1}^{\lfloor \log_p(N)\rfloor} \Bigl\lfloor \frac{N}{p^e}\Bigr\rfloor$ in $O(N^{\nicefrac{1}{2}+\epsilon})$ time.  For $p < M$ we expect to have
\[
\sum_{k=0}^{r-1}f_N(x_k)v_p(f_t(x_{k+1})) \approx \frac{1}{p-1}\int_M^t \frac{N}{x\log x}dx \approx\frac{\log 2}{p-1} < \frac{1}{p-1} \approx v_p(N!),
\]
which motivates our observation (a) above. While out algorithm operates on the assumption that \eqref{eq:rval} is nonnegative for all $p<M$, it verifies this assumption at runtime (this verification did not fail in any of our computations, but if it did one could simply increase $M$).
...
We now recall observations (b) and (c) above, that the $m_i$ are nonincreasing and $p_i$-smooth.  This implies that for very $j>i$ we have $m_j\ge m_i$, and that $m_j$ is also $p_i$-smooth, hence a suitable choice for $m_i$.  This means that we can  precompute a table of $M$-smooth integers $m\le t$ and process them in increasing order as we consider decreasing primes $p_i$, thereby obtaining a quasilinear running time $O(N^{1+\epsilon})$; note that at each step the algorithm will use the largest exponent $e$ such that $(m_p p)^e$ divides the remaining part of the factorization of $N!$, at which point one of $p$ or $m_p$ is eliminated, and the total number of $p$ and $m_p$ to be considered is $O(N)$. The running time is then dominated by the time to precompute the prime factorizations of all the candidate $m_i$.  The additional constraint on the prime factors of the $m_i$ noted in (c) reduces the number of candidate cofactors $m$ we need to store in memory by a logarithmic factor.  This does not significantly change the asymptotic time and space complexity of the algorithm, which remains quasi-linear, but it is significant in the practical range of interest, where it reduces the memory required by up to a factor of about 40 for $N\le 10^{11}$.

\subsection{A fast variant of the greedy algorithm}\label{sec:fast-greedy}
We now describe a variant of the greedy algorithm that produces slightly weaker bounds on $t(N)$ in general, but obtains an $O(N^{\nicefrac{3}{4}+\epsilon})$ running time using $O(N^{\nicefrac{2}{3}})$ space (in fact the space can be reduced to $O(N^{\nicefrac{1}{2}+\epsilon})$).  The algorithm fixes $M$ satisfying $M(M-1)\ge t$ and treats the large primes $p\ge M$ exactly as in the first phase of the greedy algorithm described above, using $O(N^{\nicefrac{3}{4}+\epsilon})$ time and $O(N^{\nicefrac{2}{3}})$ space.

In the second phase, rather than precomputing a list of all candidate $m_i\le N$, the algorithm instead precomputes a list of $M$-smooth integers $m\le N^{\nicefrac{2}{3}}$.  As it considers the small primes $p_i < M$ in decreasing order, it will eventually reach appoint where no precomputed value of $m$ is a suitable cofactor for $p_i$ (this will certainly happen for $p_i < N^{1/3}$).  When this occurs it will instead look for a cofactor that is suitable for $p_i^2$, which will be smaller and easier to construct from the remaining part of the factorization of $N!$, allowing the algorithm to remove all but at most one factor of $p_i$.  It will continue in this faction to consider suitable cofactors for both $p$ and $p^2$ until it eventually reaches a point where neither can be found, at which point all the remaining $p_i$ are either very small, $O(N^\epsilon)$, or occur with multiplicity 1.  In the final phase we simply construct factors of $N!$ larger than $t$ by combining available remaining primes in decreasing order.  This will occasionally result in factors that are substantially larger than the standard greedy algorithm would use, but there are only a small number of these and the algorithm can construct them quickly using very little memory.  This allows it to handle large values of $N$ much more efficiently, as can be seen in in the timings in Table~\ref{greedy-table}.

For this fast variant of the greedy algorithm, the computation is overwhelmingly dominated by the first phase, which takes $O(N^{\nicefrac{3}{4}+\epsilon})$ time to handle the primes $p_i>M$. The rest of the algorithm takes only $O(N^{\nicefrac{2}{3}+\epsilon})$ time, which is negligible by comparison and a substantial improvement over the quasi-linear running time of the original greedy algorithm.

\subsection{Optimizing bounds on $t(N)$ produced by the greedy algorithm}
On inputs $N$ and~$t$ the greedy algorithm produces a lower bound on $M(N,t)$. If this lower bound is greater than or equal to $N$ we can deduce $t(N)\ge t$, but it may be possible to prove a better lower bound on $t(N)$ using a larger value of $t$. This is desirable even if the goal is just to prove $t(N)\ge N/3$ (where one might simply use $t=\lceil N/3\rceil$) since the bound $t(N)\ge (N+\delta)/3$ implies $t(N')\ge N'/3$ for all $N'\in [N,N+\delta]$ (this range may include $N'$ for which the greedy algorithm cannot directly prove $t(N',t)\ge N'/3$ for any choice of $t$ with $N'$ fixed).

For large $N$ we expect to be able to choose $t$ so that the greedy algorithm (and its fast variant) can prove $M(N,t)\ge N$, and therefore $t(N)\ge t$, with $t=\lceil N+\delta)/3\rceil$ for some $\delta \ge cN$ for some $c>0$ that approaches $1/e-1/3$ as $N\to \infty$.  In the context of proving Theorem~\ref{main}, this allows us to establish $t(N)\ge N/3$ for all $N$ in any dyadic interval using just $O(1)$ runs of the greedy algorithm, provided we are able to choose suitable values of $t$.
...
\end{itemize}
Our solution to (a) suffices to establish Theorem~\ref{main}(iii) for $N\in [10^6,10^{12}]$ using the fast variant of the greedy algorithm.
Our solution to (b) allows us to extend this range to $[67425,10^{12}]$.  There are smaller $N$ for which $t_1(N)\ge N/3$, the least of which is $N=44716$, but we have $t_1(67424) < 67424/3$ so there is no way to extend the lower end of the range using the greedy algorithm.

A simple solution to (a) uses a bisection search: pick initial values $t_{\rm low} = N/4$ and $t_{\rm high} > N/2$ for which we know $[t_{\rm low},t_{\rm high})$ contains $[t_0(N),t_1(N)]$ invoke the greedy algorithm (or its fast variant) with $t=\lceil(t_{\rm low}+t_{\rm high})/2\rceil$ and update $t_{\rm low}$ or $t_{\rm high}$ as appropriate, depending on whether the greedy algorithm proves $M(N,t)\ge N$ or not.  After $O(\log N)$ iterations we will have $t_{\rm low} = t_{\rm high}-1$, at which point we know $t_{\rm low}\in [t_(N),t_1(N)]$.

We can do slightly better by using the value bound on $M(N,t)$ determined by the greedy algorithm in each iteration to guide the search, rather than simply checking whether it is above or below $N/3$.  Rather than choosing $t=\lceil(t_{\rm low}+t_{\rm high})/2\rceil$, we start with $t=\lceil N/3\rceil$ and in each iteration we replace the most recently tested value of $t$ with the nearest integer to
\[
...
(we use $(3t_{\rm low}+t_{\rm high})/4$ if it is below the interval and $(3t_{\rm low}+t_{\rm high})/4$ if it is above).  We then update $t_{\rm low}$ and $t_{\rm high}$ as above.
In practice this heuristic method converges about twice as fast as a standard bisection search.

We now consider the more challenging problem of computing $t_1(N)$.  It is not clear that this function can be computed in quasi-linear time.  In the worst case our approach potentially involves $O(N)$ calls to the greedy algorithm, whereas our solution to (a) uses only $O(\log N)$.  This limits the range of its applicability, but it is easy to parallelize the search, and this makes it feasible to compute $t_1(N)$ for $N$ as large as $10^9$ (but $N=10^{12}$ is likely out of reach).

To compute $t_1(N)$ we first use our solution to (a) to establish a strict lower bound for $t_1(N)$.  To obtain an upper bound, we use the first phase of the greedy algorithm to compute the number $B_1$ of factors $m_ip_i$ divisible by primes $p_i\ge M\approx N^{1/2}$, along with the remaining factor $R=N!/\prod\mathcal B$ expressed in terms of its valuation at primes $p < M$ via \eqref{eq:rval}.
We may then take $B_1 + \lfloor \log R / \log t\rfloor$ as a crude upper bound on the number of factors the greedy algorithm could possibly produce. We can than vary $t$ and use a bisection search to establish an upper bound on $t_1(N)$.

Computing the lower and upper bounds on $t_1(N)$ involves only $O(\log N)$ calls the the greedy algorithm (or just its first phase) and can be done quickly.  But the interval determined by these lower and upper bounds is fairly large and appears to grow linearly with $N$.  We cannot apply a bisection search because there will typically be many $t$ in this interval for which the greedy algorithm produces more than $N$ factors that are interspersed with $t$ for which this is not the case.  Lacking a better alternative, we use an exhaustive search (which can easily be run in parallel on multiple cores) to find the largest such $t$; this gives us the value of $t_1(N)$.

By using the greedy method, \Cref{main}(ii) can be verified for $N \leq 3 \times 10^5$, and \Cref{main}(iii) can be verified for $67425 \leq N \leq 10^{12}$ {\bf provide more details and links to code}.    Thus, to resolve these claims, it remains to only establish \Cref{main}(iii) in the regime $43632 \leq N < 67425$ and $N > 10^{12}$, and also to show that this claim fails for $N=43631$.
...
With integer programming, we could also establish\footnote{Explicit factorizations in this range can be found at \url{https://github.com/teorth/erdos-guy-selfridge/tree/main/Data/factorizations}.} $t(N) \geq N/3$ for all $43632 \leq N \leq 8 \times 10^4$. 
  In particular, when combined with the greedy algorithm computations from the previous section, this resolves \Cref{main}(ii), (iii) except in the asymptotic range $N > 10^{12}$, where it suffices to establish the lower bound $t(N) \geq N/3$.
...
In this section we describe an approach to establishing lower bounds on $t(N)$ by starting with the standard factorization $\{1,\dots,N\}$, dividing out some small prime factors from some of the terms, and then redistributing them to other terms.  This approach was introduced in \cite{guy} to give lower bounds of the shape $\frac{t(N)}{N} \geq \frac{3}{16} + o(1)$ (by redistributing powers of two only) and $\frac{t(N)}{N} \geq \frac{1}{4} + o(1)$ (by redistributing powers of two and three).  With computer assistance, we are also able to show that $\frac{t(N)}{N} \geq \frac{1}{3}+o(1)$ for sufficiently large $N$, in a simpler fashion than the method used to prove \Cref{main}(iv) in the next section.
...
for any $r \geq 1$.  From this one can readily check that the hypotheses of \Cref{asym-crit} are satisfied, and so we recover the bound $\frac{t(N)}{N} \geq \frac{3}{16}-o(1)$ from \cite{guy}.
\end{example}

If one makes the ansatz $a_{2^r} = c / 2^r$ for some $c>0$ and all $r \geq r_0$, with $a_\ell = 0$ for all other $\ell > 2^{r_0}$, then the task of locating weights $a_r$ obeying the hypotheses of \Cref{asym-crit} becomes a linear programming problem.  
...
\begin{proposition}  Let $N \ge 1328148$.  Then $t_{2,3}(N) < N/4$.
\end{proposition}
...
c_2 - \frac{c_3}{2} - \sum_{\ell} w_\ell \sum_{d \in \N^{\langle 2,3 \rangle}: d < 4\ell} \left( \frac{1}{d} - \frac{1}{4\ell} \right)$$
 ...
 Thus, once one produces weights for which $\eps>0$, one obtains a contradiction for $N > C/\eps$.
  Numerical computation reveals that if one selects\footnote{These coefficients were discovered by applying a linear program to a finite truncation of the problem, combined with trial-and-error experimentation to optimize the threshold.} $c_2 = 2/32$, $c_3 = 3/32$, $w_1 = 2/32$, and $w_\ell = 1/32$ for all $3$-smooth $1 < \ell \leq 2^2 3^9$ with $\nu_2(\ell) \leq 2$, with $w_\ell=0$ otherwise, then  \eqref{ell-cond} holds for all $3$-smooth $\ell$ (note that this is automatic once $\nu_2(\ell)$ or $\nu_3(\ell)$ is large enough, so this is a finite check) with
$$\eps = \frac{218038591}{4458050224128} > 0$$
and
$$ C = \frac{1559}{24}$$
and one then we obtain the desired contradiction for $N \geq 1328148 > \frac{C}{\eps}$.

{\bf Update these numbers with the additional $1$ error term (done!), also link to code}
\end{proof}
